...
public class VerifyEmailAuthenticatorCommand: IRequest, ISecuredOperation 
    // Eğer kullanıcının giriş yapmış bir application'da (örn. react) ancak doğrulamasını istiyorsak ISecuredOperation ekleyebiliriz. UserId'i JWT'den alıcaz. Fakat bu eklenen ekstra bir önlem. Genel e-posta doğrulamalarında Activation Key'in gönderilmesi yeterli olur, ayrıcı login'e ihtiyaç duymaz.
{
    ...
    public class VerifyEmailAuthenticatorCommandHandler : IRequestHandler<VerifyEmailAuthenticatorCommand>
    {
        ...
        public async Task<Unit> Handle(VerifyEmailAuthenticatorCommand request, CancellationToken cancellationToken)
        {
            UserEmailAuthenticator? userEmailAuthenticator =
                await _userEmailAuthenticatorRepository.GetAsync(uea => uea.UserId == request.UserId &&
            ...
            await verifyEmailAuthenticator(userEmailAuthenticator!); // Uzun command'ler için bu şekilde refactor işlemi yapabiliriz. Bu command uzun değil ama yine de örnek olması için bu şekilde burayı bıraktım.
